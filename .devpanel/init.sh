...
if [ -z "$(drush status --field=db-status)" ]; then
  ...
  if [ -z "${DRUPALFORGE_DEVCONTAINER:-}" ] && [ "${IS_DDEV_PROJECT:-}" != "true" ]; then
    time drush -n si drupal_cms_installer installer_site_template_form.add_ons=mercury_demo
  else
    until time drush -n si drupal_cms_installer installer_site_template_form.add_ons=mercury_demo; do
      :
    done
  fi
